...
extension TerminalView {
    ...
    {
        guard !terminal.isDisplayingSynchronizedInactiveBuffer else { return }
        let displayBuffer = terminal.displayBuffer
...
#if os(iOS) || os(visionOS)
            // The row did not change, but the fractional drag offset was
            // cleared above. Keep UIKit's content offset in lockstep so a
            // later output-driven update does not snap back to that fraction.
            updateScroller()
#endif
        }
    }
